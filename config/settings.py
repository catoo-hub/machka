...
class Settings(BaseSettings):
    ...
    WEB_PANEL_ENABLED: bool = Field(default=True, description="Enable built-in admin web panel")
    WEB_PANEL_HOST: str = Field(default="0.0.0.0")
    WEB_PANEL_PORT: int = Field(default=8081)
    WEB_PANEL_BASE_PATH: str = Field(default="", description="Base path prefix for the web panel routes (e.g. /admin)")
    WEB_PANEL_SESSION_SECRET: Optional[str] = Field(default=None, description="Secret key for signing web panel sessions")
    WEB_PANEL_LOGIN: str = Field(default="admin")
    WEB_PANEL_PASSWORD_HASH: Optional[str] = Field(default=None, description="bcrypt hash for panel password")
    WEB_PANEL_PASSWORD_PLAIN: Optional[str] = Field(default=None, description="Optional plain password (will be hashed on startup if hash missing)")
    WEB_PANEL_THEME: str = Field(default="dark")
    WEB_PANEL_CSRF_SECRET: Optional[str] = Field(default=None, description="Secret key for panel CSRF tokens")
    WEB_PANEL_JWT_SECRET: Optional[str] = Field(default=None, description="Secret key for signing JWT download links")
    ...
    # Secondary support bot configuration
    SUPPORT_BOT_TOKEN: Optional[str] = Field(default=None)
    SUPPORT_BOT_USERNAME: Optional[str] = Field(default=None)
    SUPPORT_FORUM_CHAT_ID: Optional[int] = Field(default=None)
    SUPPORT_ADMINS_STR: Optional[str] = Field(default=None, alias="SUPPORT_ADMIN_IDS")
    SUPPORT_FAQ_URL: Optional[str] = Field(default=None)
    SUPPORT_RATE_LIMIT_SECONDS: int = Field(default=15)
    ...
    @computed_field
    @property
    def support_admin_ids(self) -> List[int]:
        ids: List[int] = []
        if self.SUPPORT_ADMINS_STR:
            for raw_id in self.SUPPORT_ADMINS_STR.split(','):
                raw_id = raw_id.strip()
                if not raw_id:
                    continue
                try:
                    ...
                    logging.warning("Invalid support admin id provided: %s", raw_id)
        return ids
...
def get_settings() -> Settings:
    ...
    if _settings_instance is None:
        ...
        if not _settings_instance.WEB_PANEL_PASSWORD_HASH and not _settings_instance.WEB_PANEL_PASSWORD_PLAIN:
            logging.warning(
                "WEB_PANEL_PASSWORD_HASH is not set. Provide WEB_PANEL_PASSWORD_HASH or WEB_PANEL_PASSWORD_PLAIN for web panel login security."
            )
        if _settings_instance.WEB_PANEL_PASSWORD_PLAIN and not _settings_instance.WEB_PANEL_PASSWORD_HASH:
            logging.info(
                "WEB_PANEL_PASSWORD_PLAIN provided without hash; it will be hashed on startup."
            )
        if _settings_instance.WEB_PANEL_PASSWORD_HASH and _settings_instance.WEB_PANEL_PASSWORD_PLAIN:
            logging.warning(
                "Both WEB_PANEL_PASSWORD_HASH and WEB_PANEL_PASSWORD_PLAIN are set. Plain password will be ignored."
            )
        if _settings_instance.WEB_PANEL_ENABLED and not _settings_instance.WEB_PANEL_SESSION_SECRET:
            logging.warning(
                "WEB_PANEL_SESSION_SECRET is not set. Falling back to autogenerated secret each run (sessions will be reset on restart)."
            )
        if _settings_instance.WEB_PANEL_ENABLED and not _settings_instance.WEB_PANEL_CSRF_SECRET:
            logging.warning(
                "WEB_PANEL_CSRF_SECRET is not set. Falling back to autogenerated secret each run (CSRF tokens will be invalidated on restart)."
            )

        if _settings_instance.SUPPORT_BOT_TOKEN and not _settings_instance.SUPPORT_BOT_USERNAME:
            logging.warning("Support bot token provided but username missing (SUPPORT_BOT_USERNAME). Features may break.")
        if _settings_instance.SUPPORT_BOT_USERNAME and not _settings_instance.SUPPORT_BOT_TOKEN:
            logging.warning("Support bot username provided but token missing (SUPPORT_BOT_TOKEN). Support bot cannot start.")
